...
- (id)init{
    if ((self = [super initWithFrame:(CGRect){0,0,320,320}])){
        //DebugLog(@"[self getAvailableFrame] = %@", NSStringFromCGRect([self getAvailableFrame]));
        //DebugLog(@"cameraType = %lu", _cameraType);
...
- (void)showLiveCameraPreview:(AVCaptureDevice*)_device{
    //DebugLog(@"self.layer %@  self.bounds: %@", self.layer, NSStringFromCGRect(self.bounds));
